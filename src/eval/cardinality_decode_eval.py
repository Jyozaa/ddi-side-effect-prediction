...
@torch.no_grad()
def predict_outputs(model, samples, n_labels, device, batch_size):
    model.eval()
    all_probs, all_card, all_y = [], [], []
    ...
    for batch_idx in iter_index_minibatches(len(samples), batch_size):
        ...
        logits, card = model(g1, g2, aux1, aux2, n_graphs=y.size(0))
        probs = torch.sigmoid(logits).cpu().numpy()
        all_probs.append(probs)
        all_card.append(card.cpu().numpy())
        all_y.append(y.cpu().numpy())

    P = np.concatenate(all_probs, axis=0)
    C = np.concatenate(all_card, axis=0).reshape(-1)
    Y = np.concatenate(all_y, axis=0)
    return Y, P, C


def decode_with_predicted_cardinality(probs: np.ndarray, card_pred: np.ndarray) -> np.ndarray:
    n, L = probs.shape
    pred = np.zeros_like(probs, dtype=np.int32)

    k_vec = np.rint(card_pred).astype(int)
    k_vec = np.clip(k_vec, 0, L)
...
def main(cfg_path: str = "configs/default.yaml") -> None:
    ...
    ckpt_path = run_dir / "gnn_multilabel_cardinality" / "best.pt"
    ...
    Yv, Pv, Cv = predict_outputs(model, val_samples, n_labels, device, batch_size)
    Yt, Pt, Ct = predict_outputs(model, test_samples, n_labels, device, batch_size)

    pred_val = decode_with_predicted_cardinality(Pv, Cv)
    pred_test = decode_with_predicted_cardinality(Pt, Ct)
    ...
    results = {
        "val_micro_f1@predicted_cardinality": float(val_micro_f1),
        "test_micro_f1@predicted_cardinality": float(test_micro_f1),
        "avg_val_predicted_cardinality": float(np.mean(Cv)),
        "avg_test_predicted_cardinality": float(np.mean(Ct)),
        "n_labels": int(n_labels),
        "split_dir": str(run_dir),
        "checkpoint": str(ckpt_path),
    }

    out_path = run_dir / "gnn_multilabel_cardinality" / "cardinality_decode_results.json"
