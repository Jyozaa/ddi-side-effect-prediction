...
from src.eval.multilabel_metrics import compute_multilabel_metrics
...
def tune_global_threshold_micro_f1(y_true: np.ndarray, probs: np.ndarray) -> tuple[float, float]:
    best_t = 0.5
    best_f1 = -1.0
...
def main(cfg_path: str = "configs/default.yaml") -> None:
    ...
    print(f"Valid samples: val={len(val_samples)}, test={len(test_samples)}")
    ...
    global_t, val_best_micro_f1 = tune_global_threshold_micro_f1(Yv, Pv)
    print(f"Best global threshold on val (micro-F1): {global_t:.2f}")
    print(f"Best val micro-F1 with global threshold: {val_best_micro_f1:.6f}")

    thresholds = np.full((n_labels,), global_t, dtype=np.float32)
    ...
    results = {
        ...
        "val_micro_f1@global": float(val_metrics["micro_f1@tuned"]),
        ...
        "test_micro_f1@global": float(test_metrics["micro_f1@tuned"]),
        "n_labels": int(n_labels),
        "split_dir": str(run_dir),
        "checkpoint": str(ckpt_path),
    }

    out_path = run_dir / "gnn_multilabel_fusion" / "global_threshold_results.json"
